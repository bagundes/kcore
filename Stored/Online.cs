...
namespace KCore.Stored
{
    public static class Online
    {
        ...
        private static string dbase => Shell.FormatString.JavaScriptFormat(R.CustomerName);
        ...
        public static C.Database.Actions Save<T>(T model) where T : KCore.Base.BaseModel
        {
            try
            {
                var db = client.GetDatabase(dbase);
                var collection = db.GetCollection<T>(model.LOG);
